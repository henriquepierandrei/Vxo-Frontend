...
const LinksCard = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <motion.div
...
const FaviconImage = ({ url, domain, size = 32 }: { url: string; domain: string; size?: number }) => {
...
const LinkItem = ({
  ...
  return (
    <motion.div
      ...
      className={`group flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 p-3 sm:p-4 rounded-[var(--border-radius-md)] bg-[var(--color-surface)] border border-[var(--color-border)] hover:bg-[var(--color-surface-hover)] hover:border-[var(--color-primary)]/30 transition-all duration-300 ${disabled ? "opacity-50 pointer-events-none" : ""}`}
    >
...
const EmptyState = () => (
  ...
  >
    ...
    <h3 className="text-base sm:text-lg font-semibold text-[var(--color-text)] mb-2">Nenhum link adicionado</h3>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-[var(--color-text)]">Link de rede social detectado</p>
          <p className="text-xs text-[var(--color-text-muted)] mt-1">
            O link que você está tentando adicionar pertence ao{" "}
            <span className="font-semibold text-amber-400">{networkName}</span>. Links de redes sociais predefinidas
            devem ser adicionados na seção de <span className="font-semibold text-amber-400">Redes Sociais</span>.
          </p>
        </div>
      </div>
      <motion.button
        onClick={() => navigate("/dashboard/socialmedia")}
...
const DashboardLinks = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        ...
        setFormErrors({ url: err.response?.data?.message || "Erro ao adicionar link" });
      }
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setErrorMessage(err.response?.data?.message || "Erro ao excluir link");
      setIsDeleteModalOpen(false);
      setDeletingLink(null);
    } finally {
      setIsSubmitting(false);
    ...
    <div className="min-h-screen bg-[var(--color-background)] pb-8">
      ...
      <AnimatePresence>
        {errorMessage && (
          ...
          >
            ...
            <button onClick={() => setErrorMessage("")} className="p-1 rounded-full hover:bg-red-500/20 text-red-400">
              <X size={16} />
            </button>
          </motion.div>
        )}
          ...
          <LinksCard>
            ...
            <div className="space-y-4">
              ...
              <motion.button
                ...
                className="w-full px-4 py-3 rounded-[var(--border-radius-md)] bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white font-medium text-sm sm:text-base transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      ...
      >
        <div className="space-y-4">
          ...
          <AnimatePresence>
            {editForm.url && isValidUrl(editForm.url) && editForm.url !== editingLink?.url && !editDetectedSocial && (
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-4 rounded-[var(--border-radius-md)] bg-red-500/10 border border-red-500/30">
            ...
            <div>
              <p className="text-sm text-[var(--color-text)]">Tem certeza que deseja excluir este link?</p>
              <p className="text-xs text-[var(--color-text-muted)] mt-1">Esta ação não pode ser desfeita.</p>
            </div>
          </div>
