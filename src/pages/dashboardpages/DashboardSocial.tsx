...
// ═══════════════════════════════════════════════════════════
// ÍCONES (SVGs vazios para não pesar - substitua pelos seus)
// ═══════════════════════════════════════════════════════════
...
const SocialNetworkButton = ({ network, onClick, isAdded }: {
  ...
  <motion.button
    onClick={onClick}
    className={`relative flex flex-col items-center justify-center gap-2 p-3 sm:p-4 rounded-[var(--border-radius-md)] border transition-all duration-300 ${isAdded
...
const UserSocialCard = ({ userNetwork, network, onEdit, onDelete, disabled }: {
  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-[var(--border-radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] hover:bg-[var(--color-surface-hover)] transition-all duration-300 group ${disabled ? "opacity-50 pointer-events-none" : ""
        }`}
    >
...
// ═══════════════════════════════════════════════════════════
// COMPONENTE PRINCIPAL - USANDO O CONTEXTO
// ═══════════════════════════════════════════════════════════
...
const DashboardSocial = () => {
  ...
  // ✅ FILTRAR APENAS REDES SOCIAIS (hasLinkTyped === true)
  const socialNetworks: UserSocialNetwork[] = useMemo(() => {
    return contextLinks
      .filter((link) => link.hasLinkTyped) // ✅ Apenas links tipados (redes sociais)
      .map((link) => {
        const socialConfig = detectSocialNetwork(link.url);
        return {
          id: link.id,
          networkId: socialConfig?.id || "unknown",
          url: link.url,
          linkTypeId: link.linkTypeId,
        };
      })
      .filter((item) => item.networkId !== "unknown"); // Remove links sem rede detectada
  }, [contextLinks]);
  ...
  // ✅ ADICIONAR/EDITAR usando o contexto
  const handleSubmit = async () => {
    ...
    try {
      ...
      const typeId = detectedConfig.typeId; // ✅ typeId da rede social

      if (editingNetwork) {
        // ✅ ATUALIZAR usando o contexto
        await updateLink(editingNetwork.id, normalized, typeId);
        setSuccessMessage("Rede social atualizada!");
      } else {
        // ✅ ADICIONAR usando o contexto
        await addLink(normalized, typeId);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteLink(deletingNetwork.id); // ✅ Usando o contexto
        ...
        <motion.div variants={itemVariants} className="xl:col-span-1">
          <SettingsCard>
            <SectionHeader
              ...
              action={
                <motion.button
                  onClick={refreshLinks} // ✅ Usando o contexto
            ...
            {!isLoadingLinks && socialNetworks.length > 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={isSubmitting}
                  className={`w-full px-4 py-3 pl-10 rounded-[var(--border-radius-md)] bg-[var(--color-surface)] border transition-all duration-300 text-[var(--color-text)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/50 ${formErrors.url
