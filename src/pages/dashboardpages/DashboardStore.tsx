  ...
  if (item.type === "badge") {
    ...
    return (
      ...
      >
        ...
        <FileCode className={`${config.icon} relative z-10`} style={{ color: "white", filter: `drop-shadow(${getRarityGlow(item.rarity)})` }} />
      </div>
    );
  }
...
const LoadingSkeleton = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
    {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
      <div
        key={i}
        className="h-[400px] bg-[var(--color-surface)] rounded-[var(--border-radius-lg)] border border-[var(--color-border)] animate-pulse"
      />
    ))}
  </div>
);
...
const EmptyState = ({ type }: { type: StoreItemType | "all" }) => {
  const messages = {
    all: { title: "Nenhum item disponível", description: "Não há itens disponíveis na loja no momento." },
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 min-h-[400px]">
      ...
      <h3 className="text-base sm:text-lg font-semibold text-[var(--color-text)] mb-2">{message.title}</h3>
      <p className="text-xs sm:text-sm text-[var(--color-text-muted)] text-center max-w-xs">{message.description}</p>
    </div>
  );
};
    ...
    >
      ...
      >
        ...
        <button
          ...
          className={`p-1.5 rounded-full backdrop-blur-sm transition-all flex-shrink-0 ${item.isFavorite ? "bg-yellow-500/90 text-white" : "bg-black/30 text-white/70 hover:text-white"
            }`}
        >
        ...
        {isLocked && (
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-10">
            <div className="text-center">
              ...
              <p className="text-white text-xs font-semibold">Esgotado</p>
            </div>
          </div>
        )}
        <ItemMediaPreview
      ...
      <div className="p-4 space-y-3">
        <div>
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className="font-semibold text-sm text-[var(--color-text)] line-clamp-1 flex-1">{item.name}</h3>
            <TypeIcon size={16} className="text-[var(--color-text-muted)] flex-shrink-0" />
          </div>
          <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: rarityColor }}>
                  ...
                  <div className="flex items-baseline gap-1">
                    {item.discount ? (
                      <>
                        ...
                        <span className="text-sm font-bold text-[var(--color-text)]">{finalPrice.toLocaleString()}</span>
                      </>
                    ) : (
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)] pb-8">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm text-[var(--color-text-muted)] mb-3 sm:mb-4 overflow-x-auto whitespace-nowrap pb-2">
          <span>Dashboard</span>
      ...
      <AnimatePresence>
        {error && (
          ...
          >
            ...
            <button onClick={handleClearError} className="p-1 rounded-full hover:bg-red-500/20 text-red-400">
              <X size={16} />
            </button>
          </motion.div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-[var(--color-text-muted)]">{selectedItem.description}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 rounded-[var(--border-radius-md)] bg-[var(--color-surface)]">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-[var(--color-text)] flex items-center gap-1">
                    ...
                    {selectedItem.price === 0 ? "Grátis" : selectedItem.discount ? (
                      <span className="flex items-center gap-2">
                        <span className="line-through text-[var(--color-text-muted)]">{selectedItem.price.toLocaleString()}</span>
                        <span className="text-green-400 font-bold">{calculateDiscount(selectedItem.price, selectedItem.discount).toLocaleString()}</span>
                      </span>
              ...
              <div className="flex gap-3 pt-4">
                {!selectedItem.isOwned ? (
                  <button
                    onClick={() => { setIsPreviewModalOpen(false); handlePurchaseClick(selectedItem); }}
